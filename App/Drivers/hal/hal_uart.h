...
typedef struct hal_uart_drv_imp_s
{
    ...
    hal_uart_drv_t (*open)(hal_uart_dev_interface_t interface, const hal_uart_cfg_t *cfg);
    void (*close)(hal_uart_drv_t dev);

    uart_status_t (*write)(hal_uart_drv_t dev, const uint8_t *data, size_t len, size_t *written, uint32_t timeout_ms);
    uart_status_t (*read)(hal_uart_drv_t dev, uint8_t *data, size_t maxlen, size_t *read, uint32_t timeout_ms);

    void (*flush)(hal_uart_drv_t dev);

    void (*set_event_cb)(hal_uart_drv_t dev, hal_uart_event_cb_t cb, void *ctx);
